...
static char umm_mallo_heap[UMM_MALLOC_CFG__HEAP_SIZE] = {0};
...
void umm_init( void ) {
  ...
  umm_heap = (umm_block *)umm_mallo_heap;
...
{
    return
        (char *)ptr >= (char*) umm_heap &&
        (char *)ptr <  (char*) umm_heap + umm_numblocks * sizeof(umm_block);
}
