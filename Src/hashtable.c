...
void *pvPortCalloc( size_t num, size_t size )
{
	...
	pvReturn = pvPortMalloc(size*num);
	memset(pvReturn, 0, size*num);
...
#define	CONFIG_ENV_MIN_ENTRIES 1
#endif
...
int hcreate_r(size_t nel, struct hsearch_data *htab)
{
	...
	htab->table = (_ENTRY *) pvPortCalloc(htab->size + 1, sizeof(_ENTRY));
	if (htab->table == NULL)
		return 0;
...
void hdestroy_r(struct hsearch_data *htab)
{
	...
	for (i = 1; i <= htab->size; ++i) {
		if (htab->table[i].used > 0) {
			...
			vPortFree((void *)ep->key);
			vPortFree(ep->data);
		}
	}
	vPortFree(htab->table);
 ...
 */
static int _compare_and_overwrite_entry(ENTRY item, ACTION action,
	...
	unsigned int hval, unsigned int idx)
{
	if (htab->table[idx].used == hval
		...
		if ((action == ENTER) && (item.data != NULL)) {
			...
			vPortFree(htab->table[idx].entry.data);
		...
		int crlf_is_lf, int nvars, char * const vars[])
{
	...
	if ((data = pvPortMalloc(size)) == NULL) {
	...
	do {
		...
		*sp++ = '\0';	/* terminate value */
		...
		if (*name == 0) {
			debug("INSERT:%s","unable to use an empty key\n");
			return 0;
		}
